        ...
        <div className="mt-10 divide-y divide-border">
          {dict.faq.items.map((item, i) => (
            <details key={item.q} className="group py-5" open={i === 0}>
              <summary className="flex cursor-pointer list-none items-center justify-between text-base font-bold text-fg marker:content-none">
                {item.q}
                <svg
                  className="ml-4 shrink-0 text-green-deep transition-transform group-open:rotate-180 dark:text-gold"
                  width="18"
                  height="18"
                  viewBox="0 0 24 24"
                  fill="none"
